You drop a ball from a given height. After each bounce, the ball returns to some fixed proportion of its previous height. If the 
ball bounces to height 1 or less, we consider it to have stopped bouncing. Return the number of bounces it takes for the ball to 
stop moving.
...
function bouncingBall(initialHeight, bouncingProportion) {
	...
	while (initialHeight>1){
		initialHeight*=bouncingProportion; 
		bounce++; 
	}
	return bounce; 
}
